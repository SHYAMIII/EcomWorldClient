...
const ProductList = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("/api/products"); // Your endpoint must return an array of products
  ...
  if (isLoading)
    return <p className="w-full text-center p-10 text-white">Loading...</p>;
  if (!products.length)
    return <div className="w-full text-center p-10 text-white">No products available.</div>;
          ...
          {products.map((product, index) => (
            ...
            >
              <div className="relative h-48 w-full">
                ...
                />
                {/* Optional gradient overlay for a more aesthetic look */}
